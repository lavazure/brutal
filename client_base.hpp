   ...
   public:
    ...
    void init() {
        ...
        m_endpoint.set_message_handler(
            bind(&type::on_message, this, websocketpp::lib::placeholders::_1, websocketpp::lib::placeholders::_2));
    ...
    bool has_connection() { return m_endpoint.get_con_from_hdl(m_hdl)->get_state() == websocketpp::session::state::open; }
    ...
    void close() { m_endpoint.close(m_hdl, "", websocketpp::close::status::normal); }

    std::string get_server() { return utils::get_server(m_country, m_endpoint.is_secure()); }
    ...
    void set_timeout(std::function<void(void)> callback, unsigned long time) {
        ...
        timer->async_wait([this, timer, callback](const websocketpp::lib::error_code& ec) {
            if (!ec) {
                try {
                    callback();
    ...
    uint16_t get_player_id() { return m_player_id; }

    void run() { m_endpoint.run(); }

    // handlers
    void set_open_handler(open_handler callback) { m_open_handler = callback; }

    void set_close_handler(close_handler callback) { m_close_handler = callback; }

    void set_error_handler(error_handler callback) { m_error_handler = callback; }

    void set_message_handler(message_handler callback) { m_message_handler = callback; }

    void set_enter_game_handler(enter_game_handler callback) { m_enter_game_handler = callback; }

    void set_map_handler(map_handler callback) { m_map_handler = callback; }

    void set_death_handler(death_handler callback) { m_death_handler = callback; }

    void set_kill_handler(kill_handler callback) { m_kill_handler = callback; }

    void set_king_handler(king_handler callback) { m_king_handler = callback; }

    void set_leaderboard_handler(leaderboard_handler callback) { m_leaderboard_handler = callback; }

    void set_pong_handler(pong_handler callback) { m_pong_handler = callback; }
    ...
    void set_entity_delete_handler(entity_delete_handler callback) { m_entity_delete_handler = callback; }

    // setters
    void set_logging_id(uint16_t id) { m_logging_id = id; }

    void set_reconnect_after(unsigned long time) { m_reconnect_after = time; }

    void stop_reconnecting() { m_reconnect_after = 0; }

    void set_nick(std::u16string nick) { m_nick = nick; }

    void set_address(std::string address) { m_address = address; }

    void set_proxy(std::string proxy) { m_proxy = proxy; }

    void set_country(std::string country) { m_country = country; }

    void set_origin(std::string origin) { m_origin = origin; }

    // toggles
    void enable_autoplay() { m_autoplay = true; }

    void disable_autoplay() { m_autoplay = false; }

    void enable_logging() { m_nolog = false; }

    void disable_logging() { m_nolog = true; }

    // utils
    std::string ured(std::string text) { return "\033[91m" + text + "\033[0m"; }

    std::string ugreen(std::string text) { return "\033[92m" + text + "\033[0m"; }

    std::string uyellow(std::string text) { return "\033[93m" + text + "\033[0m"; }

    std::string ucyan(std::string text) { return "\033[96m" + text + "\033[0m"; }
    ...
    // getters
    endpoint_type& get_endpoint() { return m_endpoint; }

    io_context& get_io_context() { return m_endpoint.get_io_context(); }

    connection_hdl get_handle() { return m_hdl; }
